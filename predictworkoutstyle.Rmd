...
cv.confusion.matrix <- lapply(l.predictions,function(df) confusionMatrix(df$pred,df$obs))
cv.confusion.matrix
```

## Confusion Matrix on Cross Validation Data
```{r crossValidation}
test.predictions <- predict(myModel,df.cv)
confusionMatrix(test.predictions, df.cv$classe)
```
